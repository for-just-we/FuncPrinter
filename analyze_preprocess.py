...
def generate_preprocess_cmd_for_cmake(cmd, file_name):
    arguments = cmd.split(' ')[1:]
    new_argument = ['clang', '-E']
    output_file = ""
    for i in range(len(arguments)):
        arg = arguments[i]
        if arg == "-o":
...
def main():
    ...
    for entry in data:
        ...
        try:
            ...
            # compile_databases.json generated by bear + make
            if "arguments" in entry.keys():
                preprocess_args = generate_preprocess_cmd(entry.get("arguments"), file_name)
            # compile_databases.json generated by cmake
            else:
                assert "command" in entry.keys()
                preprocess_args = generate_preprocess_cmd_for_cmake(entry.get("command"), file_name)
